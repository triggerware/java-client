 ...
 *  a 'tuple' of values, just like a 'tuple' in a resultset returned from a one-state query. The subscription's generic parameter
 *  specifies the java class into which the tuple of values in a notification should be deserialized and made available to the
 *  subscribing client.
 ...
 * to the client in these cases, and those notfications will be handled as just described above.  
 ...
 * </p><p> An alternative is to group one or more Subscriptions in a {@link BatchSubscription}. When activated for batch notification,
 ...
 * @author nmg
 * @param <T> <p>The Java class that will be used to represent a single tuple of values that triggers the subscription's condition.
 * Each tuple arriving in a notification from the subscription will deserialize into a freshly allocated instance of this class.
 * This class cannot be a primitive java type.  Often it will be a Java 'bean' with suitable annotations for deserialization.
 * A tuple is serialized by the TW server as a json array whose elements are determined by the subscription's triggering condition.
 * </p><p>
 * The class may be Object[], or some generic Collection class with Object as the type parameter (e.g., ArrayList&lt;Object&gt;), in
 * which case a tuple will be deserialized as an Array or Collection whose elements are the default deserialization for whatever
 * Json text appears in the tuple's serial representation.
 *</p><p>
 ...
 possible to provide a more specific tuple type for a Subscription than simply Object[] -- usually one that is far more useful
...
public abstract class Subscription<T> implements NotificationInducer{
	...
	private static final NamedParameterRequest<Void> subscribeRequest = 
			new NamedParameterRequest<Void>(Void.TYPE, null, "subscribe", 
					new String[] {"description", "language", "package", "label", "method"}, new String[] {"combine"});
	private static final NamedParameterRequest<Void> unsubscribeRequest = 
	...
	private final String triggeringCondition;
	private final String notificationTag = TriggerwareClient.nextNotificationMethod("sub");
	...
	protected final TypeReference<T> tupleTypeRef;
	protected final JavaType tupleJType;
	private Object deserializationType; //set when the client owning the subscription is known

	private final Object tupleSignature;
	
	/**
	 * @param tupleType  the class of the notifications from this subscription
	 ...
	 */
	public Subscription(Class<T> tupleType, String triggeringCondition, String schema) {
		this((Object)tupleType, triggeringCondition, schema, Language.SQL);	}
	 ...
	 * @param schema the default schema for the triggeringCondition
	 */
	public Subscription(TypeReference<T> tupleType, String triggeringCondition, String schema) {
		this((Object)tupleType,  triggeringCondition, schema, Language.SQL);}

	/**
	 * This constructor is for reflective applications, where the tuple type is not known at compile time.
	 * @param tupleType  the class of the notifications from this subscription
	 * @param triggeringCondition  the two-state SQL condition for the subscription
	 * @param schema the default schema for the triggeringCondition
	 */
	public Subscription(JavaType tupleType, String triggeringCondition, String schema) {
		this( (Object)tupleType,  triggeringCondition, schema, Language.SQL);	}
	 ...
	 */
	public Subscription(Class<T> tupleType, String triggeringCondition, String schema, String language) {
		this((Object)tupleType, triggeringCondition, schema, language);	}
	 ...
	 * @param language the appropriate members of  {@link Language} for the triggeringCondition
	 */
	public Subscription(TypeReference<T> tupleType, String triggeringCondition, String schema, String language) {
		this((Object)tupleType,  triggeringCondition, schema, language);}
	 ...
	 * @param language the appropriate members of  {@link Language} for the triggeringCondition
	 */
	public Subscription(JavaType tupleType, String triggeringCondition, String schema, String language) {
		this( (Object)tupleType,  triggeringCondition, schema, language);	}
	/**
	 * @param tupleType  the type for the notifications from this subscription
	 * @param triggeringCondition  the two-state condition for the subscription
	 * @param schema the default schema for the triggeringCondition
	 * @param language the appropriate members of  {@link Language} for the triggeringCondition
	 */
	@SuppressWarnings("unchecked")
	public Subscription(Object tupleType, String triggeringCondition, String schema, String language) {
		this.triggeringCondition = triggeringCondition;
		this.schema = schema;
		this.language = language;
		if (tupleType instanceof Class<?>) {
			tupleClass = (Class<T>) tupleType;
			tupleTypeRef = null;
			tupleJType = null;
		} else if (tupleType instanceof TypeReference<?>){//probably will remove this option.
			tupleClass = null;
			tupleTypeRef = (TypeReference<T>) tupleType;
			tupleJType = null;
		} else {//uses a JavaType
			tupleClass = null;
			tupleTypeRef = null;
			tupleJType = (JavaType) tupleType;
		} 
		this.tupleSignature = tupleType;
		namedParameters = new NamedRequestParameters()
	...
	private void setDeserializationType(TriggerwareClient client) {
		...
		if (tupleClass != null) {
			deserializationType = tf.constructParametricType(SubscriptionNotification.class, tupleClass);
		} else if (tupleTypeRef != null){
			deserializationType = tupleTypeRef ;
		} else {//uses a JavaType
			deserializationType = tf.constructParametricType(SubscriptionNotification.class, tupleJType);
		} 
	...
	/*T deserializeTuple(ArrayNode jtuple, Connection connection) { 
		var twom = connection.getStreamMapper();
		if (tupleClass != null)
		  return twom.convertValue(jtuple, tupleClass);
		if (tupleTypeRef != null)
			return  twom.convertValue(jtuple, tupleTypeRef);
		return null;
	}*/
	
	private String getDispatchString() {return notificationTag;}
	
	Object getTupleSignature(){return tupleSignature;}
	/**
	...
	/*private Object computeDeserializationType(TriggerwareConnection connection) {
		...
		if (tupleJType != null)
			return tf.constructParametricType(PolledQueryNotification.class, tupleJType);
		else if (tupleClass != null) 
			return tf.constructParametricType(PolledQueryNotification.class, tupleClass);
		else return tupleTypeRef; //hope to eliminate this
	}*/
	...
	void registerWithTW(TriggerwareConnection connection) throws JRPCException {
		...
		subscribeRequest.execute (connection, namedParameters);
	 ...
	 * @param tuple the object allocated to hold the 'tuple' of values that represents a triggering of the 
	 ...
	 * subscription's condition.
	 */
	public void handleNotificationFromBatch(T tuple) {handleNotification(tuple);}

	@SuppressWarnings("unchecked")
	void handleBatchNotifications(java.util.Collection<?> tuples) {
		for (var tuple : tuples) {
			handleNotificationFromBatch((T)tuple);	}
	}
}
