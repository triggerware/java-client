...
public class IncomingMessage implements IncomingJRPCMessage{
	...
	private Object deserializeOneParameter(JsonParser jParser,  Object target) throws IOException {
		if (target instanceof Class<?>)
			return JRPCStreamParser.deserializeParameter(jParser, (Class<?>)target);
		if (target instanceof TypeReference<?>)
			return jParser.readValueAs((TypeReference<?>) target);
		if (target instanceof JavaType)
			return conn.getObjectMapper().readValue(jParser, (JavaType) target);
		//should never occur
		Logging.log("invalid type in parameter signature %s", target);
		return JRPCStreamParser.deserializeParameter(jParser, Object.class);		
	}
	...
	void deserializeNotificationParams(JsonParser jParser, ObjectMapper mapper) {
		try {
			NotificationInducer inducer = conn.getAgent().getNotificationInducer(methodName);
			...
			if (notificationType instanceof Class<?>)
				notification = (Notification)jParser.readValueAs((Class<?>)notificationType);
			else if (notificationType instanceof TypeReference<?>)
				notification = jParser.readValueAs((TypeReference<?>)notificationType);
			else {
				notification = mapper.readValue(jParser, (JavaType)notificationType);
			}
	...
	void deserializeRequestParams(JsonParser jParser) {
		try {
			...
			if (requestSignature instanceof PositionalRequestSignature) {
				final var types = ((PositionalRequestSignature)requestSignature).getParameterTypes();
				...
				if (jParser.currentToken() != JsonToken.START_ARRAY) {
					...
					for (var type : types) {
						...
						positionalParams[i] = deserializeOneParameter(jParser, /*conn,*/ type);
						i++;
					}
					if (tooFew)
						throw new JRPCRuntimeException.DeserializationFailure(
				...
				} else if (requestSignature instanceof JsonObjectRequestSignature) {//should be true if it was not positional!
